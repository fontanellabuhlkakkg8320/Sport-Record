...
export default function MemberCard({ member, onCheckin, onUpdateWeight, onViewDetail }) {
  ...
  return (
    <article className="soft-card rounded-[1.75rem] bg-white p-5">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">本周已打卡 {member.weeklyCheckins} 天</p>
          </div>
        </div>
      ...
      <div className="mt-5">
        ...
        <div className="h-3 rounded-full bg-sky-100">
          <div className="h-3 rounded-full bg-gradient-to-r from-sky-400 to-cyan-300" style={{ width: `${stats.progress}%` }} />
        </div>
      </div>
...
function Metric({ icon, label, value, highlight = false }) {
  return (
    <div className="rounded-2xl bg-sky-50/80 p-3">
      ...
      <div className={`font-bold ${highlight ? 'text-cyan-600' : 'text-slate-800'}`}>{value}</div>
    </div>
  );
}
...
function ActionButton({ label, onClick, primary = false, icon }) {
  return (
    <button onClick={onClick} className={`flex min-h-11 items-center justify-center gap-1 rounded-2xl px-2 text-xs font-bold transition active:scale-95 ${primary ? 'bg-sky-500 text-white shadow-lg shadow-sky-200' : 'bg-sky-50 text-sky-700'}`}>
      {label}{icon}
    </button>
  );
}
