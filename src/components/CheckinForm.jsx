...
export default function CheckinForm({ members, selectedMemberId }) {
  ...
  return (
    <section className="soft-card rounded-[2rem] bg-white p-6">
      <div className="mb-5 rounded-[1.5rem] bg-sky-50 p-4">
        <p className="text-sm font-semibold text-sky-600">记录今天的小坚持</p>
        <h2 className="text-2xl font-extrabold text-slate-800">每日打卡</h2>
        <p className="mt-1 text-sm text-slate-500">体重可不填，先把运动完成感记录下来。</p>
      </div>
      <form className="grid gap-5 sm:grid-cols-2" onSubmit={(event) => event.preventDefault()}>
        ...
        <button className="sm:col-span-2 flex min-h-14 items-center justify-center gap-2 rounded-[1.25rem] bg-gradient-to-r from-sky-500 to-cyan-400 font-bold text-white shadow-lg shadow-sky-200">
...
function Field({ label, children, wide = false }) {
  return <label className={`grid gap-2 text-base font-bold text-slate-700 ${wide ? 'sm:col-span-2' : ''}`}>{label}{children}</label>;
}
